...
export default function QuoteCalculator(){
 ...
 return <div className="tool"><h3>Instant Quote Calculator</h3><p>Use this for a quick estimate. Final pricing may vary based on distance, document type, parking, urgency, and appointment details.</p>
  ...
  {service==="mobile"&&<><label>Estimated travel fee</label><select value={travel} onChange={e=>setTravel(e.target.value)}><option value="25">$25 nearby / basic mobile appointment</option><option value="35">$35 standard DFW travel</option><option value="50">$50 extended DFW travel</option><option value="75">$75 urgent / longer distance</option></select></>}
  <label>After-hours or urgent appointment?</label><select value={afterHours} onChange={e=>setAfterHours(e.target.value)}><option value="no">No</option><option value="yes">Yes, add estimate</option></select>
  <label>Add-on services estimate</label><select value={addons} onChange={e=>setAddons(e.target.value)}><option value="0">No add-ons</option><option value="10">$10 printing / scan-back estimate</option><option value="20">$20 document handling estimate</option><option value="35">$35 expanded add-on estimate</option></select>
  <div className="result">Estimated total:<div className="quote-total">${total}</div><span className="small">Estimate only. Confirm final price before booking.</span></div>
 </div>
}
